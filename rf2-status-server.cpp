...
void getScoringInfo(struct mg_connection *nc, struct http_message *hm)
{
	...
	if (currentScoringInfo != NULL)
	{
		...
		for (long i = 0; i < currentScoringInfo->mNumVehicles; ++i)
		{
			...
			cJSON_AddItemToObject(v, "mLocalVel_X", cJSON_CreateNumber(vinfo.mLocalVel.x));
			cJSON_AddItemToObject(v, "mLocalVel_Y", cJSON_CreateNumber(vinfo.mLocalVel.y));
			cJSON_AddItemToObject(v, "mLocalVel_Z", cJSON_CreateNumber(vinfo.mLocalVel.z));
